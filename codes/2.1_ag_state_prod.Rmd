...
mmp_state_weather = 
  mmp_state_weather %>% 
  mutate(tavg = (tmax_avg + tmin_avg)/2)
...
## Normalize Data
As before, we need to "standarized" our dataset. We do so by taking the historical average and standard over the period 1985-2019:
```{r}
start_yr_hist = 1985
end_yr_hist   = 2019
...
We then proceed by taking the deviations for the z-scores of precipitation and temperature
```{r}
mmp_state_weather_yr = mmp_state_weather_yr %>% 
  left_join(weather_norm, by = c("municipio", "state"))

mmp_state_weather_yr = 
  mmp_state_weather_yr %>% 
...
We load the agricultural data. 

First, let's load the total plantation and harvested area:
```{r}
...
prcp_harv_plt = ggplot(mmp_state_ag %>% filter(harvested_ha != 0) %>% 
  ...
  labs(x = "Precipitation - Deviation (z-score)", 
       y = "Harvested Area - Deviation Historical Mean (%)") + 
...
tmax_harv_plt = ggplot(mmp_state_ag %>% filter(harvested_ha != 0) %>% 
  ...
  geom_point(shape = 15, size = 3, 
             color = "black")  + 
  geom_smooth(color = "darkgreen") + 
  labs(x = "Max Temperature - Deviation Z-score", 
       y = "Harvested Area - Deviation Historical Mean (%)") +
  geom_vline(xintercept = 0) + 
...
```{r}
prcp_tmax_plt = ggarrange(prcp_harv_plt + 
                            ggtitle("Precipitation and Harvested Land") +
                            labs(x = "Precipitation - Deviation (Z-score)"), 
                          tmax_harv_plt + 
                            labs(y = "", x = "Max Temperature - Deviation (Z-score)") +
                            ggtitle("Max Temperature and Harvested Land"), 
...
tmin_harv_plt = ggplot(mmp_state_ag %>% filter(harvested_ha != 0) %>% 
  ...
  geom_point(shape = 15, size = 3, 
             color = "black")  + 
  geom_smooth(color = "darkgreen") + 
  labs(x = "Min Temperature - Deviation (Z-score)", 
       y = "Harvested Area - Deviation Historical Mean (%)") +
  geom_vline(xintercept = 0) + 
...
setFixest_dict(c(`log(harvested_ha)` = "Log (Harv Area) - Ha",
                 prcp_y_mean_dev   = "Dev Precipiation (Z-score)", 
                 tavg_y_mean_dev   = "Dev Avg Temp (Z-score)",
                 tmax_y_mean_dev   = "Dev Max Temp (Z-score)",
                 tmin_y_mean_dev   = "Dev Min Temp (Z-score)"))
...
mmp_state_ag = 
  mmp_state_ag %>%
  mutate(across(ends_with("y_mean_dev"), 
                ~ .x^2, 
                .names = "{.col}_2"))
```
...
```{r}
formula = "log(harvested_ha) ~ prcp_y_mean_dev + prcp_y_mean_dev_2  | id + year"
regression = feols(as.formula(formula), 
      data = mmp_state_ag, se = 'cluster', cluster = "id")
regression_list_tot = list(regression)

for (i in c("tavg", "tmax", "tmin")) {
formula = paste0("log(harvested_ha) ~ ",i,"_y_mean_dev", 
                 " + " , i , "_y_mean_dev_2 ", "| id + year")
regression = feols(as.formula(formula), 
      data = mmp_state_ag, se = 'cluster', cluster = "id")
regression_list_tot = c(regression_list_tot,list(regression))
}
...
setFixest_dict(c(`log(harvested_ha)` = "Log (Harv Area) - Ha",
                 ...
                 tmax_y_mean_dev   = "Dev Max Temp (Z-score)",
                 tmin_y_mean_dev   = "Dev Min Temp (Z-score)", 
                 prcp_y_mean_dev_2   = "Dev Precipiation Sq (Z-score)", 
                 tavg_y_mean_dev_2   = "Dev Avg Temp Sq (Z-score)",
                 tmax_y_mean_dev_2   = "Dev Max Temp Sq (Z-score)",
                 tmin_y_mean_dev_2   = "Dev Min Temp Sq (Z-score)"))

etable(regression_list_tot[1:4], 
       file = paste0(dir$results,"harv_state_results_nl.txt"),
       fitstat=c('n', 'ar2'), replace = TRUE)
...
```{r}
formula = "log(prod_corn_ton_gr) ~ prcp_y_mean_dev + prcp_y_mean_dev_2  | id + year"
regression = feols(as.formula(formula), 
      data = mmp_state_ag, se = 'cluster', cluster = "id")
regression_list_corn = list(regression)

for (i in c("tavg", "tmax", "tmin")) {
formula = paste0("log(prod_corn_ton_gr) ~ ",i,"_y_mean_dev", 
                 " + " , i , "_y_mean_dev_2 ", "| id + year")
regression = feols(as.formula(formula), 
      data = mmp_state_ag, se = 'cluster', cluster = "id")
regression_list_corn = c(regression_list_corn,list(regression))
...
etable(regression_list_corn[1:4], 
       file = paste0(dir$results,"corn_state_results_nl.txt"),
       fitstat=c('n', 'ar2'), replace = TRUE)
...
```{r}
etable(c(regression_list_tot[1:4],regression_list_corn[1:4]), 
       file = paste0(dir$results,"ag_state_results_nl.txt"),
       fitstat=c('n', 'ar2'), replace = TRUE)
```
...
## Third specification
```{r}
formula = "log(harvested_ha) ~ prcp_y_mean_dev*tavg_y_mean_dev + prcp_y_mean_dev_2 + tavg_y_mean_dev_2  | id + year"
regression = feols(as.formula(formula), 
      data = mmp_state_ag, se = 'cluster', cluster = "id")
regression_list = list(regression)


for (i in c("prod_corn_ton_gr")) {
formula = paste0("log(", i, ")", " ~ prcp_y_mean_dev*tavg_y_mean_dev + prcp_y_mean_dev_2 + tavg_y_mean_dev_2",
                 "| id + year")
regression = feols(as.formula(formula), 
      data = mmp_state_ag, se = 'cluster', cluster = "id")
regression_list = c(regression_list,list(regression))
}
```

```{r}
setFixest_dict(c(`log(harvested_ha)` = "Log (Harv Area) - Ha",
                 `log(prod_corn_ton_gr)` = "Log (Corn Prod) - Gr, Ton",
                 prcp_y_mean_dev   = "Dev Precipiation", 
                 tavg_y_mean_dev   = "Dev Avg Temp",
                 tmax_y_mean_dev   = "Dev Max Temp",
                 tmin_y_mean_dev   = "Dev Min Temp", 
                 prcp_y_mean_dev_2   = "Dev Precipiation Sq", 
                 tavg_y_mean_dev_2   = "Dev Avg Temp Sq",
                 tmax_y_mean_dev_2   = "Dev Max Temp Sq",
                 tmin_y_mean_dev_2   = "Dev Min Temp Sq"))

etable(regression_list[1:4], 
       file = paste0(dir$results,"ag_state_results_inter.txt"),
       fitstat=c('n', 'ar2'), replace = TRUE)
