---
title:    'Maximum Temperature - Daymet: Monthly'
...
dir$daymet  <- paste0(dir$data,"daymet/tmax/")
...
```{r}
tmax_na_1980 <-
   paste0(dir$daymet, "daymet_v4_tmax_monavg_na_1980.nc") %>%
   raster::stack()
```
...
```{r}
crs(tmax_na_1980) <- "+proj=lcc +lat_0=42.5 +lon_0=-100 +lat_1=25 +lat_2=60 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
```

We change the CRS of the MMP maps to match the one of the Daymet:
```{r}
mmp_state_geo = mmp_state_geo %>%
  st_transform(crs=st_crs(tmax_na_1980))
```
...
loop_extract_daymet = 
  ...
  tmax_na_loop <-
  paste0(dir, "daymet_v4_tmax_monavg_na_",i,".nc") %>%
  raster::stack()

  crs(tmax_na_loop) <- 
    "+proj=lcc +lat_0=42.5 +lon_0=-100 +lat_1=25 +lat_2=60 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"
  
  tmax_mun_loop_crop <- raster::crop(x = tmax_na_loop,
                            y = maps)

  tmax_mun_loop <- raster::mask(x = tmax_mun_loop_crop,
                            mask = maps)

  tmax_mun_extract_loop <- raster::extract(
  x = tmax_mun_loop,
  y = maps,
  fun = mean,
  na.rm = T,
  ...
  tmax_mun_extract_loop =  tmax_mun_extract_loop %>%
  st_as_sf() %>%
  st_drop_geometry()

  tmax_mun_extract_loop = pivot_longer(tmax_mun_extract_loop,
...
```{r}
write_csv(future_tmax_all, 
          paste0(dir$data,
                    "/constructed_data/weather_state_tmax_daymet.csv"))
beep()
```