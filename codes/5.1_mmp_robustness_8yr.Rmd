...
subtitle: "Robustness: 8-year windows"
---
...
First we need to "standarized" our weather data. That is, we find the historical average and standard deviation over the period 1985-2019:
```{r}
start_yr_hist = 1985
end_yr_hist   = 2019
...
We add this "normalized" dataset to the pre-existing one. Furthermore, we only keep 8-year windows backward:
```{r}
year_window = 9
...
etable(reg_list_imm[1:4],
         file = paste0(dir$results,"imm_8yr.txt"), 
       fitstat=c('n', 'ar2'),
       replace = TRUE)
...
etable(c(reg_list_imm_il[1:4],reg_list_imm_l[1:4]),
       file = paste0(dir$results,"imm_il_l_8yr.txt"),
