...
tavg_plt = ggplot(mmp_com_weather %>% 
  ...
  geom_point(aes(x = norm_dev_tavg, y = M_UL_prop, shape = "Legal"), size = 3,
             color = "red") + 
  labs(x = "Avg Temperature - Deviation Historical Mean (ºC, t-1)", 
...
tmax_plt = ggplot(mmp_com_weather %>% mutate(rank_tmax_dev = ntile(temp_max_y_mean_dev_lag, 10)) %>%
  ...
  geom_vline(xintercept = 0) + 
    geom_vline(xintercept = tmax_m_dev, 
             linetype = "dashed") + 
  labs(x = "Max Temperature - Deviation Historical Mean (ºC, t-1)", y = "Migration (% of Population)") +
  scale_shape_manual(name = "", values = c("Total" = 15, 
                                           "Illegal" = 16, 
                                           "Legal" = 17))  +
       guides(shape=
...
prcp_tmax_plt = ggarrange(prcp_plt + ggtitle("Precipitation and Migration") + labs(x = "Deviation Historical Mean (cl, t-1)"), 
                          tmax_plt + labs(y = "", x = "Deviation Historical Mean (ºC, t-1)") + ggtitle("Max Temperature and Migration"), 
...
prcp_tmax_plt = ggarrange(prcp_plt + 
                            ...
                            labs(y = "Proporción de Migrantes (% de Población)",
                                 x = "Desvío de la Media Histórica (cl, t-1)"), 
                          tmax_plt + 
                            labs(y = "", 
                                 x = "Desvío de la Media Histórica (ºC, t-1)") + 
...
tmin_plt = ggplot(mmp_com_weather %>% mutate(rank_tmin_dev = ntile(temp_min_y_mean_dev_lag, 10)) %>%
  ...
  geom_point(aes(x = norm_dev_tmin, y = M_UL_prop, shape = "Legal"), size = 3,
             color = "red") + 
  labs(x = "Min Temperature - Deviation Historical Mean (ºC, t-1)", y = "Migration (% of Population)") + 
  geom_vline(xintercept = 0) + 
...
```{r}
tavg_tmin_plt = ggarrange(tavg_plt + labs(x = "Deviation Historical Mean (ºC, t-1)") + ggtitle("Avg Temperature and Migration"), 
                          tmin_plt + labs(y = "", x = "Deviation Historical Mean (ºC, t-1)") + ggtitle("Min Temperature and Migration"), 
          ncol = 2, common.legend = TRUE, legend = "bottom")
